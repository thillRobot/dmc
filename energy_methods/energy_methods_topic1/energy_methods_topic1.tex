...
\frametitle{\sectiontitleII}
	...
	The resulting equation clearly shows the conservation of energy for a 1DOF particle moving in the direction of gravity. This derivation is often shown in more genralized form but this version is easy to understand and will work for our purposes. \vspcc

\scalebox{1.0}{$ \Delta KE +\Delta PE = 0 $} \hspcc or \hspcc \scalebox{1.0}{$ KE + PE = Constant $} \vspc

with \hspc \scalebox{1.0}{$KE=\frac{1}{2}mv^2$} \hspc and \hspc \scalebox{1.0}{$PE=mgh$} \vspcc
...
Lastly, this derivation relies on the fact that $f$ is a conservative force that can be derived from a function $V(x)$ which is defined as follows. \vspc
...
The conservation of mechanical energy can be used to derive the equations of motion for many dynamic systems.
