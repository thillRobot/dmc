...
\small
	...
	\underline{System Response in the Time Domain}
	\begin{framed}
...
Imagine you displaced the mass, and then released from rest.

\begin{framed}
Equation of Motion:
\scalebox{1}{$m\ddot{x}+kx=0$} \vspc

Initial Position:
\scalebox{1}{$x(t=0)=x(0)=x_0$} \vspc

Initial Velocity:
\scalebox{1}{$\dot{x}(t=0)=v(0)=v_0$}
\end{framed}
...
\scalebox{1}{$m\ddot{x}+kx=0\hspc,\hspccc x(0)=x_0\hspc,\hspccc \dot{x}(0)=v_0$} \vspc
